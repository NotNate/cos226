...
public class SeamCarver2 {
    ...
    public int[] findVerticalSeam() {
        ...
        int modValue = energy[0].length;
        for (int i = 0; i < distTo.length; i++) {
            distTo[i] = Double.POSITIVE_INFINITY;
            // if (i % modValue == 0 || width == 1 || height == 1) {
            //     distTo[i / modValue] = energy[i / modValue][0];
            // }
            if (isTransposed) {
                if (i % height == 0 || width == 1) distTo[i / height] =
                        energy[i / height][0];
            }
            else {
                if (i % width == 0 || height == 1) distTo[i / height] =
                        energy[i / height][0];
            }
        }
    ...
    public static void main(String[] args) {
        ...
        CreatePhoto cp = new CreatePhoto();
        cp.constructPhoto(new String[] { "test12a.txt", "6", "6" });
        SeamCarver2 carver = new SeamCarver2(cp.getPicture());
        carver.picture();
        carver.picture();
        carver.removeHorizontalSeam(new int[] { 0, 1, 2, 2, 1, 1 });
        carver.picture();
